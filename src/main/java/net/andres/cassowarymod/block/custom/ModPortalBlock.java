package net.andres.cassowarymod.block.custom;

import net.andres.cassowarymod.worldgen.dimension.ModDimensions;
import net.andres.cassowarymod.worldgen.portal.ModTeleporter;
...
public class ModPortalBlock extends Block {
    ...
    private void handleKaupenPortal(Entity player, BlockPos pPos) {
        if (player.level() instanceof ServerLevel serverlevel) {
            ...
            ResourceKey<Level> resourcekey = player.level().dimension() == ModDimensions.TEST_DIM_LEVEL_KEY ?
                    Level.OVERWORLD : ModDimensions.TEST_DIM_LEVEL_KEY;

            ServerLevel portalDimension = minecraftserver.getLevel(resourcekey);
            if (portalDimension != null && !player.isPassenger()) {
                if(resourcekey == ModDimensions.TEST_DIM_LEVEL_KEY) {
